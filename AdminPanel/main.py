...
from sqlalchemy import func, delete, or_, create_engine, insert, update, text as sql_text
...
async def _biological_list_impl(
    ...
    # Подзапрос для сортировки по имени: берём синоним с приоритетом 1 (основное название)
    primary_name_sq = (
        ...
        .order_by(object_name_synonym_link.c.priority.asc(), ObjectNameSynonym.synonym)
        .limit(1)
        .correlate(Object)
        .scalar_subquery()
    )
    ...
    for row in rows:
        ...
        entities.append({
            "id": row.id,
            "db_id": row.db_id,
            "name_ru": _primary_synonym(synonyms, props),
            "synonyms": synonyms,
...
async def biological_save(
 ...
 # 1. Сначала создаём словарь свойств
    props = _build_bio_object_properties(
        bio_type.strip(),
    ...
    # 2. ДОБАВИТЬ ЭТУ СТРОКУ: жестко фиксируем основное название
    props["Вид (русское название)"] = name_ru.strip()
    
    # 3. Создаём объект с этими свойствами
    obj = Object(
    ...
    # 4. Создаём синоним и связь (в нижнем регистре, как при импорте)
    syn = ObjectNameSynonym(synonym=name_ru.lower().strip().replace("ё", "е"), language="ru")
    db.add(syn)
    await db.flush()
    await db.execute(insert(object_name_synonym_link).values(
        object_id=obj.id,
        synonym_id=syn.id,
        priority=1  # <-- Основное название всегда имеет приоритет 1
    ))
...
async def biological_edit(request: Request, object_id: int, db: AsyncSession = Depends(get_db)):
    ...
    # Стало (добавляем сортировку по priority):
        # Получаем синонимы, отсортированные по приоритету
    ...
    available_properties = [
        {
            ...
            "json_key": PROP_BIO_TYPE if p.property_name.strip().lower() == CATALOG_BIO_TYPE
                        else _resolve_json_key(entity["object_properties"], p.property_name),
        }
...
def _primary_synonym(synonyms: list[str], object_properties: dict | None = None) -> str:
    """Выбирает основное имя объекта. Приоритет: кириллическое название из свойств > кириллический синоним."""
    # Сначала ищем в object_properties
    if object_properties:
        for key in ["Вид (русское название)", "Наименование", "Название услуги", "name_ru"]:
            if key in object_properties and object_properties[key]:
                val = str(object_properties[key]).strip()
                # Проверяем, что это кириллица
                if any('\u0400' <= c <= '\u04FF' for c in val):
                    return val
    
    # Ищем кириллическое название среди синонимов
    if synonyms:
        for syn in synonyms:
            if any('\u0400' <= c <= '\u04FF' for c in syn):
                return syn
        # Если кириллицы нет, берём первый синоним
        return synonyms[0]
...
async def geographical_save(
    ...
    # 2. Синоним-имя + link (основное название всегда priority=1, в нижнем регистре)
    syn = ObjectNameSynonym(synonym=name_ru.lower().strip().replace("ё", "е"), language="ru")
    db.add(syn)
    await db.flush()
    await db.execute(insert(object_name_synonym_link).values(
        object_id=obj.id, synonym_id=syn.id, priority=1
    ))
...
async def service_save(
    ...
    # 1. Собираем properties и жёстко фиксируем основное название
    props = _build_object_properties(
        _parse_subtypes(subtypes),
        region.strip() or None,
        exact_location.strip() or None,
        baikal_relation.strip() or None,
    )
    props["Название услуги"] = name_ru.strip()
...
async def edit_service_text_resource(
    ...
    entity_id: int = Form(...), title: str = Form(...),
    content: str = Form(None), content_key: str = Form(None), structured_json: str = Form(None),
    db: AsyncSession = Depends(get_db),
):
    if not request.session.get("user_id"):
        return RedirectResponse(url="/login")
...
async def resource_update_features(
    ...
    if not res:
        raise HTTPException(status_code=404)
    new_features = {k.strip(): v for k, v in (data.get("features") or {}).items() if v is not None and v != ""}
...
# ==========================================
# CMS: УПРАВЛЕНИЕ СИНОНИМАМИ ОБЪЕКТА
# ==========================================
from pydantic import BaseModel

class SynonymsUpdateRequest(BaseModel):
    synonyms: list[str]


def _normalize_synonym(text: str) -> str:
    """Привести синоним к единому формату: lower case, без лишних пробелов, ё→е."""
    return text.lower().strip().replace("ё", "е")


async def _update_synonyms_impl(
    db: AsyncSession,
    object_id: int,
    data: SynonymsUpdateRequest,
    object_type_id: int,
    main_name_prop_key: str,
) -> dict:
    """Общая логика обновления синонимов для всех типов объектов.
    
    Все синонимы хранятся в нижнем регистре (как при импорте через db_importer),
    чтобы поиск через ILIKE / LOWER() работал единообразно.
    """
    obj = (await db.execute(
        select(Object).where(Object.id == object_id, Object.object_type_id == object_type_id)
    )).scalar()
    if not obj:
        raise HTTPException(status_code=404, detail="Объект не найден")

    # 1. ЖЁСТКО фиксируем основное название из свойств объекта
    main_name = (obj.object_properties or {}).get(main_name_prop_key, "").strip()

    # Проверяем, что это кириллица
    if main_name and not any('\u0400' <= c <= '\u04FF' for c in main_name):
        main_name = ""

    if not main_name:
        # Фоллбэк для старых объектов: ищем кириллическое название среди синонимов
        current_syns = (await db.execute(
            select(ObjectNameSynonym.synonym)
            .join(object_name_synonym_link, object_name_synonym_link.c.synonym_id == ObjectNameSynonym.id)
            .where(object_name_synonym_link.c.object_id == object_id)
            .order_by(object_name_synonym_link.c.priority.asc())
        )).scalars().all()
        ...
        if not main_name and current_syns:
            main_name = current_syns[0]

        if not main_name:
            main_name = "Без названия"

        obj.object_properties[main_name_prop_key] = main_name
        await db.flush()

    # Нормализуем основное название
    main_name_norm = _normalize_synonym(main_name)

    # 2. Формируем желаемый список в нормализованном виде: ОСНОВНОЕ ВСЕГДА ПЕРВОЕ
    desired_norm = [main_name_norm]
    for s in data.synonyms:
        s_norm = _normalize_synonym(s)
        if s_norm and s_norm != main_name_norm and s_norm not in desired_norm:
            desired_norm.append(s_norm)

    # 3. Получаем текущие связи (ключ — нормализованный синоним)
    current_links = (await db.execute(
        select(object_name_synonym_link.c.synonym_id, ObjectNameSynonym.synonym)
        .join(ObjectNameSynonym, ObjectNameSynonym.id == object_name_synonym_link.c.synonym_id)
        .where(object_name_synonym_link.c.object_id == object_id)
    )).all()
    current_synonyms_map = {_normalize_synonym(row.synonym): row.synonym_id for row in current_links}

    # 4. Удаляем связи для синонимов, которых НЕТ в desired_norm
    for syn_norm, syn_id in current_synonyms_map.items():
        if syn_norm not in desired_norm:
            await db.execute(
                delete(object_name_synonym_link).where(
                    object_name_synonym_link.c.object_id == object_id,
                    object_name_synonym_link.c.synonym_id == syn_id
                )
            )
            usage = (await db.execute(
                select(func.count()).select_from(object_name_synonym_link).where(
                    object_name_synonym_link.c.synonym_id == syn_id
                )
            )).scalar()
            if usage == 0:
                await db.execute(delete(ObjectNameSynonym).where(ObjectNameSynonym.id == syn_id))

    # 5. Добавляем новые или обновляем приоритет существующих
    for priority, syn_norm in enumerate(desired_norm, start=1):
        existing_syn = (await db.execute(
            select(ObjectNameSynonym).where(ObjectNameSynonym.synonym == syn_norm)
        )).scalar_one_or_none()

        if not existing_syn:
            existing_syn = ObjectNameSynonym(synonym=syn_norm, language="ru")
            db.add(existing_syn)
            await db.flush()

        link_exists = (await db.execute(
            select(object_name_synonym_link).where(
                object_name_synonym_link.c.object_id == object_id,
                object_name_synonym_link.c.synonym_id == existing_syn.id
            )
        )).first()

        if link_exists:
            await db.execute(
                update(object_name_synonym_link).where(
                    object_name_synonym_link.c.object_id == object_id,
                    object_name_synonym_link.c.synonym_id == existing_syn.id
            ...
            await db.execute(
                insert(object_name_synonym_link).values(
                    object_id=object_id,
                    synonym_id=existing_syn.id,
                    priority=priority
                )
            )

    await db.commit()
    return {"ok": True, "message": "Синонимы и приоритеты обновлены"}


@app.post("/biological/{object_id}/synonyms")
async def biological_update_synonyms(
    request: Request,
    object_id: int,
    data: SynonymsUpdateRequest,
    db: AsyncSession = Depends(get_db),
    ...
    if not request.session.get("user_id"):
        return {"ok": False, "error": "unauthorized"}
    return await _update_synonyms_impl(db, object_id, data, BIO_OBJECT_TYPE_ID, "Вид (русское название)")


@app.post("/geographical/{object_id}/synonyms")
async def geographical_update_synonyms(
    request: Request,
    object_id: int,
    data: SynonymsUpdateRequest,
    db: AsyncSession = Depends(get_db),
    ...
    if not request.session.get("user_id"):
        return {"ok": False, "error": "unauthorized"}
    return await _update_synonyms_impl(db, object_id, data, GEO_OBJECT_TYPE_ID, "Наименование")


@app.post("/service/{object_id}/synonyms")
async def service_update_synonyms(
    request: Request,
    object_id: int,
    data: SynonymsUpdateRequest,
    db: AsyncSession = Depends(get_db),
    ...
    if not request.session.get("user_id"):
        return {"ok": False, "error": "unauthorized"}
    return await _update_synonyms_impl(db, object_id, data, SERVICE_OBJECT_TYPE_ID, "Название услуги")
