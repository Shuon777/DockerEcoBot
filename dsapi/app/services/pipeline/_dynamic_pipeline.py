        ...
        try:
            for naimenovanie_off in tqdm(objs): #tqdm(df_naimenovanie_off["Русское"][:1], desc="Тестирование по списку ОФФ"):
                ...
                for id_i in range(1,4):
                    list_tepmplates_question = df_template_sen.loc[df_template_sen["id"] == id_i, "Шаблон"].to_list()[:1]
                    ...
                    for question in list_tepmplates_question:
                        ...
                        #list_question = self.llm.generate_questions_llm(question.replace('<ОФФ>', naimenovanie_off), 5)
