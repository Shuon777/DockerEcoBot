...
from domain.entities import UserRequest, SystemResponse, DialogueState, CoreResponse
...
from adapters.backend.biological import handle_get_description, handle_get_picture
from adapters.backend.geospatial import (
    handle_draw_locate_map, handle_nearest, handle_objects_in_polygon,
    handle_geo_request, handle_draw_map_of_infrastructure,
)
from adapters.backend.services import handle_describe_service
...
class DialogueSystem:
    def __init__(self, provider: str = "qwen", session=None, context_manager: IContextStorage = None):
        ...
        self.action_handlers: Dict[tuple, ActionHandler] = {
    ...
    async def process_request(self, request: UserRequest) -> List[SystemResponse]:
        ...
        try:
            ...
            if debug_enabled and rewritten_query != request.query:
                debug_traces.append(f"Rewriter: <code>{request.query}</code> → <code>{rewritten_query}</code>")
            ...
            if debug_enabled:
                debug_traces.append(f"Router: {current_intent} (via {routing_source})")
    ...
    async def _run_biology_flow(self, query, request, intent, previous, debug_traces) -> List[SystemResponse]:
        current_nlu, nlu_debug = await self.biology_worker.analyze(query)
        if request.settings.get("debug_mode"):
            debug_traces.append(nlu_debug)
        ...
        if request.settings.get("debug_mode"):
            debug_traces.append(f"Context Merge: object='{final_state.object_name}' attrs={final_state.attributes}")
    ...
    async def _run_infra_flow(self, query, request, intent, previous, debug_traces) -> List[SystemResponse]:
        analysis, infra_debug = await self.infra_worker.analyze(query)
        if request.settings.get("debug_mode"):
            debug_traces.append(f"Infra NLU:\n{json.dumps(analysis.model_dump(), ensure_ascii=False)}")
        ...
        legacy_data = {
            "action": final_state.last_action or "describe",
            "primary_entity": {
                "name": final_state.object_name, "type": "Infrastructure",
    ...
    async def _run_knowledge_flow(self, query, request, intent, debug_traces) -> List[SystemResponse]:
        analysis = await self.knowledge_worker.analyze(query)
        if request.settings.get("debug_mode"):
            debug_traces.append(f"Knowledge NLU: {json.dumps(analysis.model_dump(), ensure_ascii=False)}")
        ...
        if request.query.startswith("clarify_idx:"):
            try:
                ...
                if options and idx < len(options):
                    ...
                    legacy_data = {
                        "action": "describe",
                        "primary_entity": {"name": selected, "type": "Biological"},
                        "debug_traces": [],
                    }
                ...
                logger.error(f"Technical call error: {e}")
    ...
    async def _execute_handler(self, action, e_type, legacy_data, query, request, intent, debug_traces) -> List[SystemResponse]:
        ...
        if not handler:
            return [SystemResponse(text="Обработчик не найден.", intent=intent)]
        ...
        core_resps = await handler(
            ...
            debug_mode=request.settings.get("debug_mode", False),
        )
    ...
    def _map_to_system_responses(self, core_resps: List[CoreResponse], intent: str) -> List[SystemResponse]:
        if not core_resps:
            return [SystemResponse(text="Ничего не найдено.", intent=intent)]
        results = []
