...
async def _clear_stand_lock(user_id_to_check: str, redis_client):
...
async def _notify_and_end_session(user_id: str, bot, session: aiohttp.ClientSession, redis_client):
    ...
    try:
        ...
        if get_user_settings(user_id).get("on_stand"):
            ...
            await _clear_stand_lock(user_id, redis_client)
            await bot.send_message(
                chat_id=user_id,
                text="⏳ Ваша сессия взаимодействия со стендом завершена по истечении времени.",
            )
...
async def start_stand_session(user_id: str, bot, session: aiohttp.ClientSession, redis_client) -> bool:
    if await redis_client.set(STAND_LOCK_KEY, user_id, ex=STAND_SESSION_TIMEOUT, nx=True):
        ...
        task = asyncio.create_task(_notify_and_end_session(user_id, bot, session, redis_client))
        ...
        logger.warning(f"[{user_id}] Стенд занят пользователем {locked_by_user}.")
        return False


async def end_stand_session(user_id: str, session: aiohttp.ClientSession, redis_client):
